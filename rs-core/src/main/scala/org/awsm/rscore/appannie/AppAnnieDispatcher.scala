...
class AppAnnieDispatcher extends ParsingDispatcher{
  ...
  def getData(dates: List[String], crawler: AppAnnieCrawler): StatsResponse = {
    if (dates.size == 1) {
      ...
      val xml: String = crawler.crawl(dates.head)
  ...
  private def  parseAndBuildResponse(app: String,  store: String, rankType: String,  date: String, xml: String): StatsResponse = {
    val result = new AppAnnieXMLParser().parse(xml)
